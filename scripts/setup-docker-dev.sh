...
echo ""
echo "2. コンテナに接続:"
echo "   ${GREEN}docker exec -it cc-safe-stack-dev-1 zsh${NC}"
echo ""
echo "3. Claude Code CLIの起動（コンテナ内）:"
echo "   ${GREEN}claude${NC}"
echo ""
echo "4. 開発サーバーの起動（コンテナ内）:"
echo "   ${GREEN}pnpm run dev${NC}     # React + FastAPI"
echo "   ${GREEN}pnpm run dev:all${NC} # React + FastAPI + marimo（初回は enable:marimo）"
echo "   （marimo のみ追加したい場合は別ターミナルで ${GREEN}pnpm run dev:marimo${NC}）"
echo ""
