...
- **スキーム付きURL**: `https://cdn.company.com/packages` など
- **CIDR記法**: `192.0.2.0/24` などの IP アドレス範囲
...
| 症状                                     | 原因                               | 対処法                              |
| ---------------------------------------- | ---------------------------------- | ----------------------------------- |
| `uv sync` で企業レジストリへの接続失敗   | 企業レジストリが許可リストにない   | `ADDITIONAL_ALLOWED_DOMAINS` に追加 |
| `bun install` の外部取得失敗             | NPM以外のレジストリ使用            | 使用レジストリを許可リストに追加    |
| `curl` が `icmp-admin-prohibited` で拒否 | ファイアウォールによる外部通信遮断 | 対象ドメインを許可リストに追加      |
| API 呼び出しが `Connection timed out`    | 外部API ドメインが未許可           | API ドメインを許可リストに追加      |
